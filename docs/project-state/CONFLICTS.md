...
| C-03 | maplerime 的 `tag=1/2` 易被解释成现成的包/流双轨选路或接收能力标签。 | WS-06 只把 tag 作为工作负载标签送到选路入口；四基线不按 tag 改路由。`lb_mode=16` 在参考仓库中对两类数据均使用 MixHash 包级选路；tag 可见不证明双轨或乱序能力。 | WS-07 再定义两类的选路、传输与接收端契约，并分别验证。 |
...
| C-13 | WS-07 四格 pilot 的逐包无背景约 4 ms MoE 尾部易被误当作背景混合造成的损害或正式算法比较。 | 四格仅有一个 trace seed；`packet0` 已为 4002.653 µs，背景交互仅 +2.038 µs（0.0509%）。OoO CNP 与源包数增加是伴随观测，长尾具体致因仍待隔离。 | 先定位 NACK/CNP/重传与计时器路径，为两类/两算法冻结共同接收语义并重跑；固定总字节设计样本和 5 seed 主实验尚未运行。见 [Handoff 08](../handoffs/2026-09-25-08-ws07-dual-track-mixtax.md)。 |
