...
| C-07 | “Queue usage”可能混用 ConWeave VOQ 与真实出口队列。 | 四模式旧审计只核验 ConWeave VOQ，历史 `_out_qlen.txt` 仍无有效统计；WS-09 新模式在 BEgressQueue 入队/出队/丢包路径记录 per-port/per-tag 排队字节，见 [Handoff 10](../handoffs/2026-09-26-10-ws09-guardhash-prototype.md)。 | 报告时注明算法模式、队列层级与采样时点；不能把 WS-09 出口队列计数回填为旧 baseline 的 MMU 历史指标。 |
...
| C-11 | 旧 GuardHash 草案主要写背景长流受 MoE 喷洒影响；ANT 聊天最先追问背景增加时 MoE 小流劣化。 | 论文项目 `docs/research/12-screenshot-lessons-and-mechanism-selection.md` 与 [ADR-006](../decisions/ADR-006-conditional-guardhash-selection.md)把 MoE 受害方向列为首要诊断，同时测背景侧代价；受害方向尚未实测定论。用户后来以 [ADR-007](../decisions/ADR-007-guardhash-prototype-before-efficacy.md) 授权先做工程原型，WS-09 已按范围完成。 | WS-10 仍须先复核现象；WS-11 才能判断类别信号是否有增量，不由工程实现推导效果。 |
...
| C-15 | WS-08 的 no-go 与用户新要求“最快推进 GuardHash/HarmGate 实现与验证”表面冲突。 | no-go 针对**当前单 seed 效果证据与正式论文主张**；用户授权的可关闭工程原型已由 WS-09 核验。两者的证据等级不同，见 [ADR-007](../decisions/ADR-007-guardhash-prototype-before-efficacy.md) 与 [Handoff 10](../handoffs/2026-09-26-10-ws09-guardhash-prototype.md)。 | WS-10 预注册现象实验，WS-11 才作等信息效果判断。保留 WS-08 负结果与 WS-09 单 seed 负向 pilot。 |
| C-16 | WS-09 的九格常规技术数据固定 `7930168f`，后续丢包分类修复与压力探针固定 `9c34945`，不能把两个 SHA 写成同一构建。 | 修复仅改先前九格未触发的队列拒绝/丢弃计数分类及运行器参数；独立压力格实际触发 MMU 准入丢包并验证守恒。各结果以对应 ID 的 metadata、配置和 raw 为准。 | 若以后按最终源码做正式效果比较，全部模式须重新固定一个共同 SHA；本轮 pilot 只作技术证据。 |
