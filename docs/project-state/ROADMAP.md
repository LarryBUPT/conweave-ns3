...
4. **WS-10 现象实验，已启动预注册：**WS-09 的单 seed 技术 pilot 数值已知，不能再称为盲于先导结果。任何新效果运行前冻结可证伪条件、预算、共同传输、固定目标/总字节、五个独立 trace seed 和判据；固定总字节 0/2/4 样本先做资源 pilot。同一 seed 内复用完全相同 trace。若缺 4/5 同向、归一化交互中位数 ≥5%、全类完成率 100% 或背景安全条件，保留负结果，不升级机制主张。
...
本路线记录研究顺序与门槛。WS-09 已按工程范围闭环，WS-10 由用户本轮要求启动；后续工作流按阶段 Handoff 与用户指示创建，不因台账状态自动创建对话。
