...
5. **WS-11 全量输入现象验证，随后条件性效果评估：**先冻结完整 16,384 条同步 MoE 与原始 0/64/128/192 条背景大流的输入哈希、共同传输、目标/背景指标、热点观测、重复 seed 和判据。原始档位总字节随背景增加，需并列设计可解释的负载或固定总字节控制；两类结果分别报告，不能把总负载增加直接归因于包/流混合。先做单格资源 pilot，审计现有运行器的单仿真限制、独立目录/缓存及远程空闲情况，再用实测资源逐级提高并发，静默完成可恢复矩阵。只有新的现象门槛通过，才比较普通短队列双候选、类别感知 GuardHash、HarmGate 门控开/关，以及始终共享、静态隔离和可核查 APS/FLB-like 强对照；同时检查 MoE、背景两侧和类别信号增量。新场景 no-go 时停止机制收益宣称，保留解释性负结果；WS-10 的 4 档 no-go 始终有效。
...
本路线记录研究顺序与门槛。WS-09 工程范围和 WS-10 预注册现象复核均已闭环；WS-11 明确补入全量验证阶段，其效果比较继续受现象门槛约束。后续工作流按阶段 Handoff 与用户指示创建，不因台账状态自动创建对话。
