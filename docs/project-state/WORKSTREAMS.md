...
| WS-07 | 包/流双轨及 MixTax | PLANNED | 下一工作流：先补 MoE `2.000s` 分组统计与共同传输/重排语义，做单类与双类正确性；再双向比较 packet/flow × 背景 0/64，并准备固定总负载、独立 trace seed 的主实验 | 依赖 WS-06；现象不成立则停止 WS-08 主线 |
| WS-08 | GuardHash/HarmGate 与强基线 | CONDITIONAL | 仅在双侧损害诊断通过后实现 GuardHash v0，检验相对普通本地短队列二选一的类别信号增量，再做强对照、消融与边界 | 只在 WS-07 的 go 门槛通过后启动；见 [ADR-006](../decisions/ADR-006-conditional-guardhash-selection.md) |
