...
| WS-11 | 全量输入现象验证；条件性强对照与效果判断 | ACTIVE FOR NEW-SCENARIO PREFLIGHT; EFFICACY GATE CLOSED | 先独立预注册完整 16,384 条同步 MoE 与原始 0/64/128/192 背景输入的现象问题、总字节变化的归因控制、热点指标和判据；做资源 pilot，再按门槛运行可恢复的多 seed 全量矩阵。只有新场景通过现象门槛，才比较普通短队列双候选、GuardHash 类别信号、HarmGate 开/关、共享/静态隔离及可核查强对照 | WS-10 当前场景 no-go 不改；先审计并发运行器、独立构建/输出和服务器空闲资源，按工作流静默运行。类别增量与双侧代价不足则保留负结果，不升级机制主张 |
| WS-12 | 边界、成本与论文证据整合 | CONDITIONAL FOR MECHANISM; EXPLANATORY ROUTE AVAILABLE | 若后续有机制增量，测错标、反馈延迟、拓扑不对称和状态/线上标签成本；目前可整理 WS-10 负结果、非单调性和接收/传输证据 | 以 WS-10/11 的原始数据和决定为入口，不把 NS-3 原型称为硬件部署 |
