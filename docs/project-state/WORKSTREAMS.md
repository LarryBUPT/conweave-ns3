...
| WS-08 | 接收门槛诊断与共同 IRN pilot | COMPLETE FOR PREFLIGHT AND SINGLE-SEED PILOT | [Handoff 09](../handoffs/2026-09-25-09-ws08-receiver-gate.md)：四个 4 ms 尾流对应末段 RTO；共享 IRN 四格交互 `−0.053 µs`、背景 P99 差 0。当前**效果主张 no-go**；固定总字节 0/2/4 和五 seed 未仿真，GuardHash 未实现 | WS-09 按 [ADR-007](../decisions/ADR-007-guardhash-prototype-before-efficacy.md) 可做工程原型；效果门槛仍沿用 [实验契约](../research/ws07-dual-track-contract.md) |
| WS-09 | GuardHash/HarmGate 最小原型与正确性 | ACTIVE FOR ENGINEERING PROTOTYPE | 从 WS-08 已推送分支起步；冻结门控/评分规格，做普通双候选短队列和类别感知模式，验证 per-port/per-tag 计数守恒、小拓扑/跨 ToR 正确性及资源受控技术 pilot；不声称收益 | 技术验证可直接开始；共同 IRN 契约、固定 SHA/trace、旧模式回归和 [ADR-007](../decisions/ADR-007-guardhash-prototype-before-efficacy.md) 为约束 |
| WS-10 | 固定总字节与多 seed 现象复核 | PLANNED | 先冻结可证伪条件、预算、目标集合与独立 trace seed；运行已静态核验的 0/2/4 固定字节样本资源 pilot，再在预注册场景做五 seed 配对。报告正、负结果与原始数据 | 不用 WS-09 的收益结果挑场景；共同传输、完成率和 WS-07 判据不变，变更需版本化 |
