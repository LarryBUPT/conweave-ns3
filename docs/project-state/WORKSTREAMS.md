...
| WS-11 | 等信息强对照、消融和效果判断 | CONDITIONAL; CURRENT GATE NOT MET | 仅对重新预注册且通过现象门槛的新场景，比较普通短队列双候选、GuardHash 类别信号、HarmGate 开/关、共享/静态隔离及可核查强对照；检验双侧代价与增量 | WS-10 当前正式场景 no-go；类别增量和稳定损害不足则保留负结果，不升级论文机制主张 |
