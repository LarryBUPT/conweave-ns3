...
**冻结的主指标与 go / no-go：**主指标是相同目标 MoE 集合的合成批次完成时间，仅在目标流 100% 完成时定义。四格绝对交互量为 `(packet64−packet0)−(flow64−flow0)`，同时以 `packet0` 批次时间归一化为百分比；背景侧主安全指标为 64 背景流 P99 FCT 及完成率。当前契约下，逐包无背景已出现约 4 ms NACK/重传尾部，因此先将“逐包无背景批次时间不应由少量乱序重传主导”作为共同接收语义前置门槛。修正或重新选择接收语义后，必须对全部算法/两类采用同一配置并重跑四格。

正式主实验预定 **5 个独立 trace seed**，每个 seed 内四格配对、同一固定总提供字节和目标集合；至少 4/5 个 seed 的交互量同向，且交互量的中位数达到 `packet0` 批次时间的 **5%**，才视为有实际意义的跨类损害。任一 seed 的任一类别未 100% 完成、背景 P99 相对全流格恶化超过 **5%**、或 MoE 差异主要由不一致的 PFC/IRN/接收配置产生，均不得进入 WS-08。PFC/CNP、源 ToR 包数和 uplink 原始记录是解释指标，不事后替换主指标。预算与 seed 列表在正式运行前一次性记录并冻结；若资源约束要求修改样本规模，重新声明设计，不沿用旧阈值解释。ConWeave 若进入导入混合延迟拓扑比较，须先审计其统一 `one_hop_delay` 推导的 ToR 时序值；当前不把它作为此双轨实验的对照。
