...
WS-06 核验补充（2026-09-24）：Windows 的 `core.autocrlf=true` 曾使这些资产在 Git blob/Linux 检出时被换行规范化，导致远程实际字节哈希不同。现已通过仓库 `.gitattributes` 对六个迁入资产设置 `-text`，并重新暂存原始 CRLF 字节；下表哈希对应版本库字节及后续 Linux 检出字节。旧的轻量探测 `20260924-220000-ws06-1280-bdp` 在修正前固定 SHA 上运行，拓扑快照哈希为 `6c100b3a3d3a75a4e7577ad57504a71e5e30087eb1b6b373be03066c1333cfae`，仅换行不同，不能与下表哈希混用。
...
1. WS-06 已实现五/六列解析、缺省 `tag=0`、显式 `--flow-file` 和到交换机选路入口的 packet tag 通路；小拓扑与 1280 拓扑的四流技术验证、旧五列四基线回归见新 Handoff。四份完整 MoE 文件尚未做大规模仿真或性能评测。
